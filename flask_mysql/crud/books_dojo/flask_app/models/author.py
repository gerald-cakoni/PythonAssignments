...
class Author:
    ...
    def get_by_id(cls,data):
        query = "SELECT * FROM authors LEFT JOIN favourites ON authors.id = favourites.author_id LEFT JOIN books ON books.id = favourites.book_id WHERE authors.id = %(id)s;"
        results = connectToMySQL('books_authors').query_db(query,data)
