...
class Post:
    ...
    @classmethod
    def get_all(cls):
        query = "SELECT posts.id, posts.user_id, posts.title, posts.content, posts.image, users.first_name, users.last_name, COUNT(likes.post_id) as num_likes FROM posts  LEFT JOIN users ON posts.user_id = users.id LEFT JOIN likes ON posts.id = likes.post_id GROUP BY posts.id;"
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO posts (title, content, image, user_id) VALUES ( %(title)s, %(content)s, %(image)s, %(user_id)s);"
